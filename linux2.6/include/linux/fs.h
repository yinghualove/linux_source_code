...
/* fs/char_dev.c */
extern int alloc_chrdev_region(dev_t *, unsigned, unsigned, char *);     //动态的申请和注册一个设备号，推荐使用
extern int register_chrdev_region(dev_t, unsigned, char *);              //静态的申请和注册设备号
extern int register_chrdev(unsigned int, const char *,                   //int为0时候动态注册，非零时候静态注册。
			   struct file_operations *);
